...
//                     $vehicleData[] = $vehicle->load('owner', 'tracker', 'lastLocation');
...
class VehicleController extends Controller
{
    public function create(Request $request)
    {
        ...
        $userCarOwner = User::find(strip_tags($request->user_id));

        $vehicleOwner = VehicleOwner::firstOrCreate(
            ['user_id' => $userCarOwner->id],
            [
                'first_name' => $userCarOwner->first_name,
                'last_name' => $userCarOwner->last_name,
                'email' => $userCarOwner->email,
                'phone' => $userCarOwner->phone,
            ]
        );
    ...
    public function viewAll()
    {
        ...
        if (in_array($user->user_type, ['individual', 'organization'])) {
            $vehicleQuery->where('user_id', $user->id);
        }

        $vehicles = $vehicleQuery->with(['owner', 'tracker', 'lastLocation'])->paginate(10);

        foreach ($vehicles as $vehicle) {
            $vehicle['driver'] = Driver::where('vehicle_vin', $vehicle->vin)->first();
        }
    ...
    public function modifyVehicle(Request $request)
    {
        ...
        if (!$user || !in_array($user->user_type, ['admin', 'system_admin']) || !$user->can('edit vehicle')) {
            return response()->json(['error' => 'Invalid Access'], 403);
        }
        ...
        $updateData = $request->only(['brand', 'model', 'year', 'vin', 'number_plate', 'type']);
        $vehicle->update(array_filter($updateData, fn($value) => !is_null($value)));
    ...
    public function deleteVehicle(Request $request)
    {
        ...
        if (!$user || !in_array($user->user_type, ['admin', 'system_admin']) || !$user->can('delete vehicle')) {
            return response()->json(['error' => 'Invalid Access'], 403);
        }
